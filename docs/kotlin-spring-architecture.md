...
| `child_database` | 데이터베이스 참조와 읽기 전용 데이터 뷰 | `table`, `list`, `gallery`만 각 레이아웃으로 표시. 나머지 뷰는 사용자 지정 지원 정책에 따라 탭과 본문에서 제외하며, 지원 뷰가 없으면 DB 영역도 제외. 행을 별도 게시글로 자동 수집하지 않음 |
| `link_to_page(database_id)` | 외부 문서 참조 카드 | `DEGRADED`; 링크만으로 데이터베이스나 그 행을 수집하지 않음 |
...
- `child_database`와 데이터베이스 행은 공개 범위를 넓히지 않는다. 인라인 표에는 원본 뷰가 반환한 행 중 `public_url`이 있고 휴지통에 있지 않은 행의 표시 속성만 포함한다.
...
`child_database`는 일반 블록 자식 조회가 아니라 데이터베이스와 Views API로 수집한다. `link_to_page(database_id)`는 수집을 유발하지 않는 참조 카드로 유지한다. 인라인 데이터베이스는 제품 정책상 `table`, `list`, `gallery`만 지원한다. 그 외의 알려진/미래 뷰 타입은 행 쿼리나 스키마 조회 전에 제외하고 안내 탭도 만들지 않는다. 이는 일반 블록의 안전한 미지원 폴백 정책과 구분되는 명시적 예외다. 모든 뷰가 제외되면 빈 탭/DB 제목 영역도 만들지 않는다. 지원 뷰 자체에 접근할 수 없거나 표시 속성을 확인할 수 없는 경우에만 안전한 링크/안내를 남긴다.

- 데이터베이스 메타데이터와 뷰 목록을 읽고 각 뷰가 해당 표시 데이터베이스에 속하는지 확인한다. 뷰가 지정한 데이터 소스의 스키마를 조회한다. 연결된 데이터 소스도 이 명시적 참조와 접근 가능 여부로 확인하며 표시 데이터베이스의 소유 소스라고 가정하지 않는다. 명시적인 표시 속성 목록이 없는 뷰는 추측해 전체 속성/행을 공개하지 않고 안전한 카드나 안내로 남긴다.
- 뷰 쿼리를 생성하고 커서를 끝까지 읽어 저장된 필터·정렬을 적용한 행 ID를 얻는다. 각 행은 표시 속성 ID와 갤러리의 명시적 커버 속성 ID만 요청하고, `public_url == null` 또는 휴지통 상태이면 스냅샷에 넣지 않는다. 커버용 숨긴 속성은 셀에 넣지 않는다. 행이나 관련 페이지를 `containedChildren`에 넣지 않는다.
- 여러 지원 뷰는 기존 탭 모델로 표현한다. 필터링 후 API 반환 순서의 첫 지원 탭을 초기 표시하며, 이를 Notion 사용자의 현재 선택/기본 뷰라고 간주하지 않는다. 표는 열별 폭·줄바꿈, 전체 줄바꿈, 고정 열 수, 세로선 설정을 반영한다. 그룹 배치·하위 항목은 평면 행으로 표시한다. 리스트는 제목과 속성 목록, 갤러리는 커버와 속성을 가진 카드 그리드로 표시한다. 셀 수정·행 추가·드래그 이동 같은 편집 기능은 제공하지 않는다.
- 소스 중립적인 `DataViewContent`를 `Table`, `ListView`, `Gallery`로 닫힌 모델링한다. 세 타입은 `DataSet`(제목·열·행·표시 제목 열 위치)을 공유하고, 표 옵션과 갤러리 옵션은 해당 하위 타입에만 둔다. `DataColumn`은 이름·선택적 의미상 픽셀 폭·줄바꿈 설정을, `DataRow`는 셀별 `InlineContent`와 선택적인 행 링크·아이콘·커버 `MediaSource`만 가진다. 열은 하나 이상이며 행의 셀 수와 일치하고 제목 열 위치와 고정 열 수는 범위를 검증한다. Notion DTO, 원본 설정 JSON, CSS 문자열을 도메인에 저장하지 않는다.
- 새 스냅샷은 논리 `data_view` kind와 명시적인 `table`/`list`/`gallery` 레이아웃을 저장한다. 이전 버전 1의 `data_table`은 스냅샷 어댑터에서 `DataViewContent.Table`로 복원하며 구 도메인 타입을 병렬로 유지하지 않는다. 이전 표 스냅샷에는 원본 뷰 타입이 없으므로 복원된 표는 다음 정상 동기화에서 새 지원 정책으로 교체한다.
- 갤러리 커버는 명시적인 `page_cover`, 파일 속성, `page_content` 첫 이미지 설정을 지원한다. 공개 행 확인 후에만 커버를 정규화한다. 파일 속성은 파일명/URL 경로의 이미지 확장자로 식별하며 원격 HEAD 요청으로 형식을 추측하지 않는다. `page_content`는 그 행의 직접 블록을 기존 페이지네이션/시간/수집량 한도 안에서 읽어 첫 이미지 하나만 쓰고, 중첩 본문이나 링크 페이지를 재귀 수집하지 않는다. 서명 URL은 `MediaSource.SourceHosted`로 만료 시각을 보존한다. 커버 없음·접근 불가·만료는 이미지 없는 카드로 표시하며 임의의 다른 이미지로 대체하지 않는다.
- 제목/텍스트, 숫자, 체크박스, 선택·상태, 날짜, URL, 이메일·전화번호, 공개 표시 이름, 파일 이름, 계산된 스칼라 값은 안전한 텍스트/리치 텍스트로 표현한다. 관련 문서, 파일 바이너리, 사용자 ID·이메일 메타데이터는 따라가지 않는다. 알 수 없는 속성과 불완전한 속성 응답은 원시 JSON이나 잘린 값을 완전한 값처럼 표시하지 않고 명시적인 미지원/불완전 안내를 제공한다.
- 뷰·행·셀 수집은 게시글의 전체 제한 시간과 수집량 제한 안에서 처리한다. 접근 불가 데이터베이스/뷰는 안전한 참조로 대체하며, 인증·일시 장애·잘못된 응답·수집 한도 초과는 동기화 실패로 분류해 기존 스냅샷을 유지한다. 불완전한 쿼리를 정상적인 전체 표로 저장하지 않는다.
- 부모 페이지 수정 시각만 같아도 DB 행·스키마·뷰가 바뀔 수 있다. 인라인 DB가 포함된 소스 버전에는 정규화된 데이터와 뷰 구성을 반영한 지문을 포함해 행 수정, 미게시 전환, 필터·열 변경이 다음 동기화에서 반영되게 한다. 원본 응답이나 비밀 값은 소스 버전에 기록하지 않는다.

표시 계층은 도메인의 세 타입에 대응하는 명시적 뷰 타입과 Thymeleaf 프래그먼트를 사용한다. 표의 caption/thead/tbody, 리스트의 목록/속성 의미, 갤러리 카드 제목·이미지 대체 텍스트와 빈 결과 안내를 제공한다. 열 폭은 고정된 CSS 폭 단계로 제한하고 고정 열의 좌측 위치는 신뢰된 자산 스크립트가 실제 DOM 폭을 측정해 계산한다. 원본 CSS나 inline script를 삽입하지 않는다. 새 버전의 CSS/JS와 추가 전용 프로필 마이그레이션으로 기존 자산을 유지한다. 코드 블록의 언어별 하이라이팅은 이 변경에 포함하지 않는다.
