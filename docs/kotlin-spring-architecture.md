...
표시 계층은 도메인의 세 타입에 대응하는 명시적 뷰 타입과 Thymeleaf 프래그먼트를 사용한다. 데이터베이스 제목은 Notion과 같이 독립된 섹션 제목으로 한 번 표시한다. 뷰 이름은 전환이 필요한 다중 뷰의 탭에서만 시각적으로 표시하며, 표 caption과 리스트·갤러리 내부 제목은 스크린 리더용 이름으로만 남겨 제목을 반복하지 않는다. 표의 caption/thead/tbody, 리스트의 목록/속성 의미, 갤러리 카드 제목·이미지 대체 텍스트와 빈 결과 안내를 제공한다. 열 폭은 고정된 CSS 폭 단계로 제한하고 고정 열의 좌측 위치는 신뢰된 자산 스크립트가 실제 DOM 폭을 측정해 계산한다. 데이터베이스 섹션은 일반 문서 참조 카드의 테두리·그림자·호버 배경을 제거하고 제목 링크와 읽기 전용 본문을 Notion 인라인 데이터베이스처럼 이어서 표시한다. 편집 전용 검색·필터·정렬·새로 만들기 컨트롤을 흉내 내지 않는다. 원본 CSS나 inline script를 삽입하지 않는다. 새 버전의 CSS/JS와 추가 전용 프로필 마이그레이션으로 기존 자산을 유지한다. 코드 블록의 언어별 하이라이팅은 이 변경에 포함하지 않는다.
