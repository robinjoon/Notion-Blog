...
이미지 발행 job은 공통 `load-ci-secrets@v1.0.0` Action으로 SMS의 `zot` 객체를 조회합니다. GitHub OIDC 인증 후 전달된 `REGISTRY_USERNAME`, `REGISTRY_PASSWORD` 환경변수를 사용하며, 같은 이름의 CI 자격증명을 앱별 GitHub Secrets에 반복 등록하지 않습니다.
