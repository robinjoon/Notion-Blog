...
class BlogWebController(
    ...
    fun page(@PathVariable slug: String): ResponseEntity<Any> =
        if (slug in reservedRootSegments) notFound() else pageResponse(pageAccess.lookup("/$slug"), root = false)
    ...
    private fun safeRedirect(status: HttpStatus, destination: String): ResponseEntity<Any> =
        if (!isSafeInternalLocation(destination)) {
            notFound()
        } else {
            runCatching { redirect(status, destination) }.getOrElse { notFound() }
        }

    private fun redirect(status: HttpStatus, destination: String): ResponseEntity<Any> =
        ResponseEntity.status(status).location(URI.create(destination)).build()

    private fun html(body: String): ResponseEntity<Any> =
        ResponseEntity.ok().contentType(MediaType.TEXT_HTML).body(body)

    private fun serviceUnavailable(): ResponseEntity<Any> =
        ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).build()

    private fun notFound(): ResponseEntity<Any> =
        ResponseEntity.status(HttpStatus.NOT_FOUND).build()

    private fun isSafeInternalLocation(destination: String): Boolean =
